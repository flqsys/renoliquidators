...
class MeowPro_MFRH_Core {
	...
	public function __construct( $core, $admin, $engine ) {
		...
		if ( $this->admin->is_registered() ) {
			...
			add_filter( 'mfrh_vision_suggestion', array( $this, 'vision_suggestion' ), 10, 3 );
			...
			add_action( 'mfrh_media_resync', array( $this, 'sync_media_meta' ), 10, 1 );
	...
	public function sync_media_meta( $post, $old_filename = null, $new_filename = null, $undo = false, $method = null )   {
		if  ( $undo ||  ( $method == 'manual' && $this->core->get_option ( 'disable_manual_sync' ) ) )  {
			return;
		}
		...
		$this->syncOptions ( $post, $method, $old_filename, $new_filename, [
			'sync_alt' => 'action_sync_alt',
			'sync_media_title' => 'action_sync_media_title',
			'sync_description' => 'action_sync_description',
			'sync_caption' => 'action_sync_caption',
		] ) ;
	}
	
	private function syncOptions ( $post, $method, $old_filename, $new_filename, $options )  {
	...
	private function action_sync_common ( $post, $method, $old_filename, $new_filename, $type ) {
		$auto_rename = $this->core->get_option ( 'auto_rename', false ) ;
		$new_value = '';
		$filter_suffix = $type;

		$new_filename = pathinfo ( $new_filename, PATHINFO_FILENAME ) ;
	
		if  ( $auto_rename && $method == 'auto' )  {
			if  ( $type == 'alt_text' )  {
		...
		} elseif  ( $auto_rename && $method == 'auto' && !$new_value )  {
	...
	function vision_suggestion( $res, $mediaId, $prompt ){
		...
		global $mwai;
		$prompt = "The tasks that follows should be largely influenced by the contents depicted in the image.\n" . $prompt;
		$prompt = apply_filters( 'mfrh_vision_prompt', $prompt );

		// Get the intermediate image details
    $intermediate = image_get_intermediate_size( $mediaId, 'medium' );

    if ( $intermediate ) {
			// Use the 'medium' size thumbnail path and URL
			$thumbnail_path = path_join( dirname( get_attached_file( $mediaId ) ), $intermediate['file'] );
			$thumbnail_url = wp_get_attachment_image_url( $mediaId, 'medium' );
    }
		else {
			// Fallback to full size if 'medium' size is not available
			$thumbnail_path = get_attached_file( $mediaId );
			$thumbnail_url = wp_get_attachment_url( $mediaId );
    }

		$res = $mwai->simpleVisionQuery( $prompt, $thumbnail_url, $thumbnail_path );
		return $res;
	}
